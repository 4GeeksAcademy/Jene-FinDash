...
export function KPICard({ label, value, helperText, icon: Icon, variant, loading }: KPICardProps) {
  ...
  const labelId = `kpi-label-${variant}`
  const valueId = `kpi-value-${variant}`
  ...
  if (loading) {
    return (
      ...
      >
        <CardContent className="kpi-card-content p-6 flex flex-col gap-4">
  ...
  return (
    <Card
      ...
      aria-describedby={`kpi-help-${variant}`}
    >
      <CardContent className="kpi-card-content p-6 flex flex-col gap-4">
